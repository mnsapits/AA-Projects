...
def vanity_projects
  ...
  Movie.select("movies.id,movies.title,actors.name").joins(:actors).where("castings.ord = 1 and movies.director_id = castings.actor_id")
...
def starring(whazzername)
  ...
  Movie.select("movies.*").joins(:actors).where("")
end

def bad_years
  # List the years in which a movie with a rating above 8 was not released.
  Movie.select("movies.yr").where("score > 8 and votes is NULL")
end
