...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
  SELECT DISTINCT
    stops.name, craig_routes.company, craig_routes.num
  FROM
    (SELECT
     a.company,
     a.num,
    ...
    JOIN
     routes b ON (a.company = b.company AND a.num = b.num)
    WHERE
     b.stop_id = '53') AS craig_routes
  JOIN
   stops
  ON
  stops.id = craig_routes.stop_id
  SQL
...
def craiglockhart_to_sighthill
  ...
  FROM
    ...
    WHERE
     b.stop_id = '53') AS craig_routes
    JOIN
    stops ON  (c.stop_id = stops.id)
  WHERE
    stops.name = 'Sighthill' AND c.num != craig_routes.num
  SQL
end
